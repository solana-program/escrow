...
/// Accounts for the BlockTokenExtension instruction
///
/// # Account Layout
/// 0. `[signer, writable]` payer - Pays for account creation/realloc
/// 1. `[signer]` admin - Must match escrow.admin
/// 2. `[]` escrow - Escrow account to add block token extensions to
...
pub struct BlockTokenExtensionAccounts<'a> {
...
impl<'a> TryFrom<&'a [AccountView]> for BlockTokenExtensionAccounts<'a> {
...
impl<'a> InstructionAccounts<'a> for BlockTokenExtensionAccounts<'a> {}
