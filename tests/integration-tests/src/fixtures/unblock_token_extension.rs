use escrow_program_client::instructions::UnblockTokenExtensionBuilder;
use solana_sdk::{
    pubkey::Pubkey,
...
pub struct UnblockTokenExtensionFixture;

impl UnblockTokenExtensionFixture {
    pub fn build_with_escrow(
        ...
        let instruction = UnblockTokenExtensionBuilder::new()
...
impl InstructionTestFixture for UnblockTokenExtensionFixture {
    const INSTRUCTION_NAME: &'static str = "UnblockTokenExtension";
    ...
    fn build_valid(ctx: &mut TestContext) -> TestInstruction {
        ...
        AddBlockTokenExtensionsFixture::build_with_escrow(ctx, escrow_pda, admin.insecure_clone(), 1u16)
